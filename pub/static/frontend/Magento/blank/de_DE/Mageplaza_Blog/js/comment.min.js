require(['jquery'],function($){var cmtBox=$('.default-cmt__content__cmt-block__cmt-box__cmt-input'),submitCmt=$('.default-cmt__content__cmt-block__cmt-box__cmt-btn__btn-submit'),defaultCmt=$('ul.default-cmt__content__cmt-content:first'),likeBtn=defaultCmt.find('.btn-like'),replyBtn=defaultCmt.find('.btn-reply');submitComment();likeComment(likeBtn);showReply(replyBtn);$('li.default-cmt__content__cmt-content__cmt-row:first').css({'border-top':'none'});$('.default-cmt__cmt-login__btn-login').click(function(){var socialPopup=$("[href$='social-login-popup']");if(socialPopup.length){socialPopup.first().trigger("click");}else{window.location.href=loginUrl;}});function checkGuestFormValidate(){if(isLogged=='No'){return $("#default-cmt__content__cmt-block__guest-form").valid();}
return true;}
...
break;}}});}
function htmlComment(text){var html='';for(var i=0;i<text.split("\n");i++){html+='<p>'+i+'</p>';}
return html;}
function displayComment(cmt,isReply){var cmtRow='<li id="cmt-id-'+cmt.cmt_id+'" class="default-cmt__content__cmt-content__cmt-row cmt-row-'+cmt.cmt_id+' cmt-row col-xs-12 '
...
return false;}});}else{cmtRow=$(cmtRow);defaultCmt.append(cmtRow);likeComment(cmtRow.find('.btn-like'));showReply(cmtRow.find('.btn-reply'));}}});