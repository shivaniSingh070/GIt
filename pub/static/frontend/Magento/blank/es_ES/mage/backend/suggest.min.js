...
option.data('renderedOption').remove();option.remove();},_renderOption:function(item){var tmpl=this.choiceTmpl({text:item.label});return $(tmpl).insertBefore(this.elementWrapper).trigger('contentUpdated').on('removeOption',$.proxy(function(e){this.removeOption(e,item);},this));}});return $.mage.suggest;});