define(['Magento_Checkout/js/model/shipping-rate-registry','Magento_Checkout/js/model/shipping-service','Magento_Checkout/js/action/select-shipping-method','Magento_Checkout/js/model/quote','uiRegistry','mage/translate','jquery'],function(rateRegistry,shippingService,selectShippingMethodAction,quote,uiRegistry,$t,$){'use strict';return function(Component){return Component.extend({successHandler:function(data){if(!data.data){return;}
...
window.checkoutConfig.selectedShippingMethod=selectMethod;}}else{shippingService.setShippingRates([]);selectShippingMethodAction(null);window.checkoutConfig.selectedShippingMethod=[];}}})};});