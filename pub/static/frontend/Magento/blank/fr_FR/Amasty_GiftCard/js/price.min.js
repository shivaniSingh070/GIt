define(['uiComponent','jquery'],function(Component,$){'use strict';return Component.extend({defaults:{prices:'',currencyCode:'',currentPrice:null,feeStatus:'',feeDisabled:'0',feeValue:null,feeValueConverted:null,productId:null,priceTypePercent:1,priceTypeFixed:2,customAmount:'',customMinAmount:'',customMaxAmount:'',customMinAmountCurrency:'',customMaxAmountCurrency:'',isValueValid:true,priceSelector:'.price',isSinglePrice:'',isLoaded:false,showCustomPrice:false,element:{priceLabel:'[data-amcard-js="price"]'},imports:{preconfiguredValues:'${ "giftCard" }:preconfiguredValues'}},initialize:function(){this._super();this.sortPrices();this.prepareCustomAmountRange();this.showCustomPrice(!this.isSinglePrice);this.isLoaded(true);return this;},initObservable:function(){this._super().observe(['prices','currentPrice','isValueValid','customAmount','showCustomPrice','isLoaded']);return this;},sortPrices:function(){this.prices().sort(function(a,b){return a.value-b.value;});},changeProductPrice:function(item){var value=parseFloat(item.convertValue);this.customAmount('');this.applyPrice(value);},applyPrice:function(value){if(this.feeStatus!==this.feeDisabled){value=this.applyingFee(value);}
...
return'';},getCardPriceValue:function(name){var price=this.getPriceValue(name);if(price){price=this.prices().filter(function(el){return el.convertValue===price;})[0];if(price!==undefined){this.currentPrice(price.value);}}},getCustomPriceValue:function(name){var price=this.getPriceValue(name);if(price){this.customAmount(price);}}});});