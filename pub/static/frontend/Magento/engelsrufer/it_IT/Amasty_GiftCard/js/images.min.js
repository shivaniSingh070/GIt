define(['uiComponent','jquery','Amasty_Base/vendor/slick/slick.min','mage/translate'],function(Component,$){'use strict';return Component.extend({defaults:{images:[],customImage:false,isCustomImageAllowed:false,errorMessage:'',checkedImageId:null,customImageFile:'',isCustomImageChecked:false,stateActive:'-active',stateAmcard:'-amcard',customImageUrl:'',firstImageIndex:0,imports:{preconfiguredValues:'${ "giftCard" }:preconfiguredValues'},element:{carousel:'[data-amcard-js="carousel"]',customImageSelector:'[data-amcard-js="custom-image"]',sliderImages:'[data-amcard-js="amcard-image"]',gallery:'[data-gallery-role="gallery"]',amGallery:'[data-role="amasty-gallery"]',amcardImage:'[data-amcard-js="card-image"]',galleryContainer:'[data-gallery-role="gallery-placeholder"]'},maxSizeUserImage:1572864,extentionsUserImage:['image/png','image/jpeg','image/gif'],incorrectSizeMessage:$.mage.__("The image couldn't be uploaded because "+"it exceeds 1,5 Mb, the maximum allowed size for uploads."),incorrectTypeMessage:$.mage.__("The image couldn't be uploaded, only files with "+"the following extensions are allowed: jpg, gif, png")},initialize:function(){this._super();$('body').trigger('processStart');this.changeSelectedImage(this.images[this.firstImageIndex].src,this.images[this.firstImageIndex].id);return this;},initObservable:function(){this._super().observe(['customImage','errorMessage','checkedImageId','customImageFile','isCustomImageChecked']).observe({isSingleImage:this.images.length===1});return this;},getPreconfiguredImage:function(){var img={};if(this.customImageUrl){img.src=this.customImageUrl;this.customImage(img);this.deleteSelectedImage();this.isCustomImageChecked(true);this.changeMainImage(img);}},validateUserImage:function(userImage){if(userImage.size>this.maxSizeUserImage)return this.incorrectSizeMessage;if(this.extentionsUserImage.indexOf(userImage.type)===-1)return this.incorrectTypeMessage;return'';},changeSelectedImage:function(src,id,element){this.checkedImageId(id);this.isCustomImageChecked(false);this.changeMainImage({src});if(element){$(this.element.sliderImages).removeClass(this.stateActive);$(element).addClass(this.stateActive);}},changeMainImage:function(newImage){var isFotoramaLoaded=this.useFotorama(newImage.src);if(!isFotoramaLoaded){$(this.element.galleryContainer).on('gallery:loaded',function(){this.useFotorama(newImage.src);}.bind(this));}
...
this.checkedImageId(imgId);imageElement=$(this.element.sliderImages+'[data-id="'+imgId+'"]');imageElement.addClass(this.stateActive);img.src=imageElement.attr('src');this.changeMainImage(img);this.getPreconfiguredImage();},initSlick:function(){$(this.element.carousel).slick({dots:false,infinite:false,slidesToShow:4,slidesToScroll:1,responsive:[{breakpoint:1300,settings:{slidesToShow:3,slidesToScroll:1}},{breakpoint:1024,settings:{slidesToShow:2,slidesToScroll:1}},{breakpoint:768,settings:{slidesToShow:4,slidesToScroll:1}},{breakpoint:425,settings:{slidesToShow:3,slidesToScroll:1}}]});}});});