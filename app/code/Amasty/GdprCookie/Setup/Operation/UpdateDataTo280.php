...
class UpdateDataTo280
{
    ...
    {
        return $this->resource->getTableName(CreateCookieConsentTable::TABLE_NAME);
    }

    private function getConsentStatusTable(): string
    {
        return $this->resource->getTableName(CreateCookieConsentStatusTable::TABLE_NAME);
    }
    ...
    private function getGroupSelect(?array $storeIds = null): Select
    {
        $select = $this->getConnection()->select()->from(
            ['g' => $this->resource->getTableName(CreateCookieGroupsTable::TABLE_NAME)],
            [CookieGroupsInterface::ID]
        )->distinct(true);

        if ($storeIds !== null) {
            $select->joinLeft(
                ['gs' => $this->resource->getTableName(CreateCookieGroupStoreTable::TABLE_NAME)],
