 ...
 */
namespace Lof\BasePrice\Helper;
...
class Data extends AbstractHelper
{
    ...
    public function getBasePriceText(Product $product)
    {
        ...
        $delivery = $this->getDelivery($product);
        if($delivery){
            $delivery = '<br/><span class="delivery">'.$delivery.'</span>';
        }
        $base_price_text = str_replace('{REF_DELIVERY}', $delivery, $base_price_text);
     ...
     * @return string
     */
    public function getDelivery(Product $product)
    {
        return $product->getData('baseprice_delivery');
    }
    ...
    public function getBasePrice(Product $product)
    {
        ...
        $customBasePrice = $product->getData('baseprice_custom_price');

        $basePrice = 0;
        if($customBasePrice){
            $basePrice = $customBasePrice;
        }elseif ($productPrice && $conversion && $referenceAmount && $productAmount && $productAmount > 0) {
