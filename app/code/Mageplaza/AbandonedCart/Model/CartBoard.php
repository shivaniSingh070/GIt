...
class CartBoard
{
    ...
    {
        $timeMeasure = $this->helperData->getRealtimeConfig('time_measure');
        $data = [];
        try {
            $data = [
    ...
    private function filterDateRange($isActive, $from = null, $toD = null)
    {
        ...
        if ($from !== null) {
            ...
            $quotes->addFieldToFilter('updated_at', ['gteq' => $startDateValue]);
        }
        if ($toD !== null) {
            $endDateValue = $this->helperData->getEndDateUTC($toD);
            $quotes->addFieldToFilter('updated_at', ['lteq' => $endDateValue]);
        }
