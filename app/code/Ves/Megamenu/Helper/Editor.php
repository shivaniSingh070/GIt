...
class Editor extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function renderCellTemplate($fieldName, $uploadImageText = "Upload Image")
    {
        ...
        if (isset($field['type'])) {
            ...
            switch ($field['type']) {
                ...
                case 'image':
                    ...
                    $tmpLinkImage = $this->_backendData->getUrl('cms/wysiwyg_images/index',
                        [
                            'target_element_id' => $editorId,
                            'as_is' => 'ves'
                        ]
                    );
                    $html .= '<input value="' . $tmpLinkImage . '" class="tmp_link_image" type="hidden"/>';
                        ...
                        "jquery"
                            ...
                            jQuery(document).ready(function($){
                                jQuery(document).on("click","#'.$buttonBlock->getId().'", function () {
                                    MediabrowserUtility.openDialog("' . $this->_backendData->getUrl('cms/wysiwyg_images/index',$fileBrowserUrlParams) . '", null, null,"' . $uploadImageText . '", "")
                                });
                            })
                        })
                    </script>
                ...
                case 'editor':
                    ...
                    $tmpLinkEditor = $this->_backendData->getUrl(
                        'vesmegamenu/product/wysiwyg'
                    );
                    $html .= '<input value="' . $tmpLinkEditor . '" class="tmp_link_editor" type="hidden"/>';
                    $html .= '<input value="' . $editorId . '" class="tmp_editor_id" type="hidden"/>';
